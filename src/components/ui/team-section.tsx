    ...
    return (
      ...
      >
        <div className="container mx-auto max-w-7xl grid items-center justify-center gap-12 px-4 text-center md:px-6">
          ...
          {/* Team Members Grid (Executives) */}
          <div className="relative z-10 mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:gap-8">
          ...
          {directors && directors.length > 0 && (
            <div className="mt-20 w-full">
              ...
              <div className="relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-8">
                {directors.map((member, index) => (
                  <DialogPrimitive.Root key={`dir-${index}`}>
                    <DialogPrimitive.Trigger asChild>
                      <div
                        className="group cursor-pointer relative flex flex-col items-center justify-end overflow-hidden rounded-3xl bg-white p-8 text-center shadow-lg transition-all duration-500 ease-in-out hover:-translate-y-2 hover:shadow-2xl border border-black/5"
                      >
                        ...
                        />
                        <div
                          className="relative z-10 h-32 w-32 overflow-hidden rounded-full border-4 border-transparent bg-[#fafafa] transition-all duration-500 ease-out group-hover:border-red-600 group-hover:scale-110 mb-6 shadow-sm"
                        >
                          <img
                            src={member.imageSrc}
                            alt={member.name}
                        ...
                        <h3 className="relative z-10 text-xl font-black font-heading text-black uppercase">
                          {member.name}
                        </h3>
                        <p className="relative z-10 text-xs font-bold tracking-widest uppercase text-red-600 mt-2">
                          {member.designation}
                        </p>
                        {member.socialLinks && member.socialLinks.length > 0 && (
                          <div className="relative z-10 mt-6 flex gap-4 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100">
